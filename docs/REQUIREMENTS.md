...
> 版本：v1.1
> 更新：2026-07-21
> 状态：Phase 5 设计文档（Phase 4.5 基础已实现：Bot 交互 + 订阅 CRUD + 热点追踪）
...
| 之前（AI 日报） | 现在（Phase 4.5 已完成） | 之后（Phase 5 🎯） |
|---|---|---|
| 看什么内容由我决定 | 用户可订阅/追踪任意话题 ✅ | LLM 辅助意图理解 + 多源自动匹配 |
| 固定 7 个 AI 源 | 按话题动态匹配信源 ✅ | 扩展新闻源（澎湃、新华社等） |
| 推送固定格式日报 | 热点→时间线 / 长期→每日简报 ✅ | 多模式智能路由 |
| 不支持用户交互 | 飞书 Bot 对话式交互 ✅ | Web 界面（可选） |
| 不能新建/取消任务 | 订阅 CRUD + SQLite 持久化 ✅ | 生命周期管理 + 已读标记 |
...
### Step 4 — Bot 完整交互（🟡 进行中 —— P0 代码完成，2026-08-09）

> P0 已实现：消息→引擎路由、订阅记录来源会话（chat_id）、node-cron 调度（`src/bot/scheduler.ts`）、默认订阅初始化。
> 待办：配置 Bot 凭据 → 本地端到端验证 → 云端部署 → 停旧 timer。
